...
options = optimoptions('fmincon', 'Algorithm', 'sqp', 'Display', 'iter', ...
                       'OptimalityTolerance', 1e-6, 'MaxFunctionEvaluations', 2000);
...
% Define the nonlinear constraints function handle
nonlcon = @(jointAngles) nonlinear_constraints(jointAngles, robot1_template, robot2_template);

% Run fmincon with nonlinear constraints
[optimal_joint_angles, fval] = fmincon(objectiveFunction, initial_joint_angles, [], [], [], [], lb, ub, nonlcon, options);
...
function [c, ceq] = nonlinear_constraints(jointAngles, robot1_template, robot2_template)
    % Create copies of the robot objects to avoid modifying shared state
    robot1 = robot1_template;
    robot2 = robot2_template;
    ...
    % Get the magnet positions from the robots
    [position1, ~] = robot1.getMagneticField();
    [position2, ~] = robot2.getMagneticField();

    % The constraint is that y-coordinate of end effector positions should be zero
    ceq = [position1(2); position2(2)]; % y-components should be zero
